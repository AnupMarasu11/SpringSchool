...
class Simulation:
    ...
    def _shift_field(self, 
        ...
        #field = field[..., self.simulation_raw_data.subject]  # apply your subject mask (boolean or indices)
        # Flatten the spatial dimensions using the mask
        masked_field = field[:, :, :, self.simulation_raw_data.subject, :]  # shape: (2, 2, 3, N, 8), where N = number of True voxels
        
        field_shift = einops.einsum(masked_field, coeffs, 'hf reim fieldxyz ... coils, hf reimout reim coils -> hf reimout fieldxyz ...')
        # Initialize output array
        output = np.zeros((2, 2, 3, *self.simulation_raw_data.subject.shape), dtype=field_shift.dtype)

        # Fill masked locations
        output[:, :, :, self.simulation_raw_data.subject] = field_shift
